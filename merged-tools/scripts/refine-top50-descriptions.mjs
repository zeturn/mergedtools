...
async function run(){
  ...
  // sort by name and pick top 50
  list.sort((a,b)=>a.name.localeCompare(b.name))
  const top = list.slice(0,50)
  console.log('Top 50 tools to refine:')
  ...
  for (const t of top) {
    ...
    const full = await readFile(t.idxPath, 'utf8')
    const newText = full.slice(0, t.block.start) + newBlockText + full.slice(t.block.end)
