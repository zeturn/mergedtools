...
export default function Page() {
  ...
  return (
    <div className="space-y-3">
      <input className="w-full rounded bg-slate-800 p-2" value={txt} onChange={(e) => setTxt(e.target.value)} placeholder="输入文本" />
      <div className="rounded bg-slate-800 p-2 font-mono break-all">MD5: {md5}</div>
      <div className="grid grid-cols-2 gap-3 items-center">
        <input className="rounded bg-slate-800 p-2" value={key} onChange={(e) => setKey(e.target.value)} placeholder="HMAC 密钥" />
