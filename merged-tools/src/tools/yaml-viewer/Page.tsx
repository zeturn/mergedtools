  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <section className="space-y-2">
        <h2 className="text-xl font-semibold">输入 YAML</h2>
...
function Tree({ node, path }: { node: any; path: string }){
  if (node === null || typeof node !== 'object') {
    return <KV k={path.split('.').pop() ?? ''} v={String(node)} type={typeof node} />
  }
...
function KV({ k, v, type }: { k: string; v: string; type: string }){
